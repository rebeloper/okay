...
| `okay:now-i-do-it` | `/okay:now-i-do-it` | Captures your current git diff, removes it, then paces you to rebuild it by hand. |
| `okay:wait-what` | `/okay:wait-what` | A directive: re-pitch the last message with context, in plain technical English. |
...
2. `okay:now-i-do-it` writes exactly two non-source things: its answer-key patch under `.okay/`, and a `.okay/` line in `.gitignore`. It runs git commands that reset the working tree. It never authors code.
